...
def get_days_left_before_expiry(days: int):
    """
    Simple method to get the days left before expiry and account disable,
    will expire in 30days of start
    default is 30days
    """
    return timezone.now() + timezone.timedelta(days=days)


def addDays(date_, num_of_days: int):
    """
    A method to add days to date should incase,
    we choose to extend days of expiry
    """
...
def get_fileType(filepath):
    """
    returns path to file in the server
    """
    import os
...
def random_string_generator(size=10, chars=string.ascii_lowercase + string.digits):
    """
    A simple random string generator to be used for almost anything
    """
